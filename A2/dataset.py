...
def dataset(dataset_name):
    ...
    if (dataset_name == 'cifar10'):
        ...
        sys.stdout.write("| ")
        trainset = torchvision.datasets.ImageFolder('./data/train', transform=transform_training())
        testset = torchvision.datasets.ImageFolder('./data/val', transform=transform_testing())
        outputs = 120
        inputs = 3
    
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=cf.batch_size, shuffle=True, num_workers=5)
    testloader = torch.utils.data.DataLoader(testset, batch_size=cf.batch_size, shuffle=False, num_workers=5)
